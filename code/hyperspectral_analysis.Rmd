...
Let's stack up all of the hyperspectral bands into a rasterstack as the raster input to this function. 
...
The accompanying *hyperspectral_analysis.R* script loops through all of the HDF5 files and extracts spectra using the steps described above. The spectra are written to separate .csv files for each tile, then combined into a single .csv tile for subsequent analysis. The code below reads the .csv file with all spectra. 
...
spectra_all <- as.data.frame(read.csv(paste0(out_dir, 
                                             site_code,
                                             "_spectral_reflectance_ALL.csv"))) %>% 
                  select(-X.1)
```
...
ggplot(data = spectra_plot, 
  ...
  ggtitle("Hyperspectral reflectance extracted per center pixel") +
          scale_color_manual(values = c("#d7191c", "#fdae61", "#1BA7C9", "#2b83ba")
  )
```
...
spectra_tree <- spectra_plot %>% 
              filter(individualID=="NEON.PLA.D13.NIWO.01131") 

# all reflectance spectra for a tree polygon
spectrum_tree <- spectra_plot %>% 
              filter(individualID=="NEON.PLA.D13.NIWO.01131") %>%
                filter(spectra_count==1)
...
ggplot(data = spectrum_tree, 
  ...
  geom_line(aes(group = individualID), size = 0.3, alpha = 0.7) + 
  labs(x = "wavelength (nm)") + 
  ggtitle("Hyperspectral reflectance for a single pixel") +
...
# shaded around it.
# shading could illustrate 95th confidence interval, min & max, ...
